...
import type { AddAlertCommentInput, AddAlertCommentOutput } from '@/types/rpc';
...
export async function handleAddAlertComment(item: OutboxItem): Promise<void> {
  const payload = JSON.parse(item.payload) as AddAlertCommentInput;
  ...
  const result = await callRpc<AddAlertCommentOutput>(
