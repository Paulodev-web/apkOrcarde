...
import type { RecordPoleEquipmentInput, RecordPoleEquipmentOutput } from '@/types/rpc';
...
/**
 * Envia um registro de equipamento montado.
 *
 ...
 */
export async function handleRecordPoleEquipment(item: OutboxItem): Promise<void> {
  const payload = JSON.parse(item.payload) as RecordPoleEquipmentInput;
  ...
  const result = await callRpc<RecordPoleEquipmentOutput>(
